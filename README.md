...
实测：一条 2 分 30 秒的 1080p30 口播（单段素材），Apple M1 Pro 16 GB，渲染并发 1。

| 阶段 | 机器时间 | 随什么变长 | 这段时间你要做什么 |
|---|---|---|---|
| 0 转写 + 抽帧 + 铺工程 | 50 秒（large-v3 转写 42 秒） | 原始素材时长 × 0.3 | — |
| 0 提案 | agent 读稿写提案 | — | **看提案、回复**：选档位、改动效位置、给图 |
| 1 剪辑拼接 + 转写母版 | 2 分钟（重编码 82 秒 + 转写 42 秒） | 成片时长 × 0.8 | — |
| 1 人物层 | 2 分 16 秒 | 成片时长 × 0.9 | — |
| 2 `npm install` | 1 分 45 秒（每个项目第一次；设了 `REMOTION_NODE_MODULES` 就是 0） | — | — |
| 2 画面结构 + 体检 | 几秒（agent 写 `structure.py` 另算） | — | — |
| 3 预览静帧 | 打包 7 秒 + 每帧 3 秒（一般 15–20 帧，约 1 分钟） | — | — |
| 3 整片渲染 | **20 分 30 秒**（pro 20:25 / max 20:30） | 成片时长 × 8 | 等着，可以去干别的 |
| 3 QA + 交付 | 1 分 13 秒 | 成片时长 × 0.5 | 看成片 |

**合计：机器时间约 30 分钟 ≈ 成片时长 × 12，渲染占七成；另加 agent 写提案 / 结构和你确认的来回。**

- 三档渲染时间几乎一样：瓶颈是逐帧截图和视频解码，不是动效多少（pro、max 实测；lite 没单独测过）。
- 阶段 0 按**原始素材**时长算（重录多、素材长就更久），阶段 1 之后按**成片**时长算。
- 提速：`CONCURRENCY=2` 能明显快，但 16 GB 内存吃紧时会出瓦片乱帧（QA 会拦下、要重渲），先关掉别的应用再开。
- 返工只改画面、不改剪辑：从阶段 2 开始，主要就是重渲那 20 分钟。

**一次性准备**（每台机器只做一次）：下载 3.1 GB 转写模型（100 Mbps 约 4 分钟）；Remotion 第一次渲染自动下载 85 MB 浏览器。冒烟测试（三档各出一片）：本机 7 分 16 秒（依赖已装好时），GitHub Actions 上 10 分 26 秒（含现场编译 whisper.cpp）。
